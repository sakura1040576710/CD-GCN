...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset',
                        required=True,
                        choices={'ntu/xsub', 'ntu/xview', 'ntu120/xsub', 'ntu120/xset', 'NW-UCLA', 'UAV/xsub1',
                                 'UAV/xsub2'},
    ...
    if 'UCLA' in arg.dataset:
        ...
        if 'xsub1' in arg.dataset:
            arg.alpha = [1, 1, 1, 1, 1.5, 1.5]
            with open('./data/' + 'uav/' + 'xsub1/' + 'test_label.pkl', 'rb') as label:
                label = np.array(pickle.load(label))
        elif 'xsub2' in arg.dataset:
            arg.alpha = [2.5, 2.5, 1.5, 1, 1, 1]
            with open('./data/' + 'uav/' + 'xsub2/' + 'test_label.pkl', 'rb') as label:
                label = np.array(pickle.load(label))
    else:
