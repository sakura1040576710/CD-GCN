...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=17, num_person=2, graph=None, graph_args=dict(), in_channels=3,
        ...
        self.l4 = TCN_GCN_unit(base_channel, base_channel, A, adaptive=adaptive, loop_times=0)
        self.l5 = TCN_GCN_unit(base_channel, base_channel * 2, A, stride=2, adaptive=adaptive, loop_times=0)
        self.l6 = TCN_GCN_unit(base_channel * 2, base_channel * 2, A, adaptive=adaptive, loop_times=0)
        self.l7 = TCN_GCN_unit(base_channel * 2, base_channel * 2, A, adaptive=adaptive, loop_times=0)
        self.l8 = TCN_GCN_unit(base_channel * 2, base_channel * 4, A, stride=2, adaptive=adaptive, loop_times=0)
        self.l9 = TCN_GCN_unit(base_channel * 4, base_channel * 4, A, adaptive=adaptive, loop_times=0)
        self.l10 = TCN_GCN_unit(base_channel * 4, base_channel * 4, A, adaptive=adaptive, loop_times=0)
...
if __name__ == '__main__':
    net = Model(graph="graph.uav.Graph",
                graph_args={"labeling_mode": 'spatial'}).cuda()
    ske = torch.rand([1, 3, 64, 17, 2]).cuda()
